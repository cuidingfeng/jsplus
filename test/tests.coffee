QUnit.test "测试 arr基础功能", ( assert ) ->
  arr1 = P.arr "1:5"
  expect 6
  assert.ok arr1.length is 5, "数组长度通过!"
  for arr,i in arr1
    assert.ok arr is i+1, "数组元素通过!"

QUnit.test "测试 arr递增值", ( assert ) ->
  arr1 = P.arr '8:0.5:12'
  assert.ok arr1.length is 9, "数组长度通过!"
  for arr,i in arr1
    assert.ok arr is i*0.5+8, "数组元素通过!"

QUnit.test "测试 arr自动判断递增方向", ( assert ) ->
  arr1 = P.arr '12:8'
  assert.ok arr1.length is 5, "数组长度通过!"
  for arr,i in arr1
    assert.ok arr is 12-i, "数组元素通过!"

QUnit.test "测试 arr小写字母", ( assert ) ->
  arr1 = P.arr 'c:f'
  assert.deepEqual arr1, ['c', 'd', 'e', 'f'], "数组完全相等通过!"

QUnit.test "测试 arr大写字母", ( assert ) ->
  arr1 = P.arr 'G:-2:A'
  assert.deepEqual arr1, ['G', 'E', 'C', 'A'], "数组完全相等通过!"

QUnit.test "测试 arr中文数字", ( assert ) ->
  arr1 = P.arr '二:5:二十'
  assert.deepEqual arr1, ['二', '七', '一十二', '一十七'], "数组完全相等通过!"

QUnit.test "测试 arr多段组合", ( assert ) ->
  arr1 = P.arr '1:3, a:c ,D:F,一:三'
  assert.deepEqual arr1, [1, 2, 3, 'a', 'b', 'c', 'D', 'E', 'F', '一', '二', '三'], "数组完全相等通过!"

QUnit.test "测试 arr表达示格式化", ( assert ) ->
  arr1 = P.arr 'a:c', 'text$1-$index'
  assert.deepEqual arr1, ['texta-0', 'textb-1', 'textc-2'], "数组完全相等通过!"

QUnit.test "测试 arr函数格式化", ( assert ) ->
  arr1 = P.arr 'a:c', (index, $1) -> index*2 + $1
  assert.deepEqual arr1, ['0a', '2b', '4c'], "数组完全相等通过!"

QUnit.test "测试 arr参数异常", ( assert ) ->
  arr1 = P.arr 'a:C'
  ...
  arr1 = P.arr 'a:b:c'
  assert.deepEqual arr1, ['a', 'c'], "递增值类型错误通过!"


QUnit.test "测试 cn2num中文转换数字", ( assert ) ->
  num1 = P.cn2num '八'
  ...
  num1 = P.cn2num '零'
  assert.strictEqual num1, 0, "数字0通过!"

QUnit.test "测试 num2cn数字转中文", ( assert ) ->
  num1 = P.num2cn 8
  ...
  num1 = P.num2cn 0
  assert.strictEqual num1, '零', "数字零通过!"

QUnit.test "测试 assign按条件返回值", ( assert ) ->
  val = P.assign [
    ["a", true]
    ["b", false]
  ]
  ...
  val = P.assign 'a'
  assert.strictEqual val, null, '参数非数组通过!'

QUnit.test "测试 inArr按条件查找数组", ( assert ) ->
  arr1 = P.arr "3:8"
  b = P.inArr arr1,'$1>5'
  ...
  b = P.inArr arr1, (index, $1, size) -> size - index > $1
  ...
  assert.strictEqual b, undefined, '返回匹配值，无匹配项通过!'

QUnit.test "测试 forArr按条件处理数组的每一项", ( assert ) ->
  arr1 = P.arr "3:5"
  ...
  arr3 = P.forArr arr1, (index, $1, size) -> index + $1 + size
  assert.deepEqual arr3, [5, 7, 9], '用函数处理正确'


  err = P.forArr "test"
  assert.deepEqual err, null, '参数错误'
